...
export function MessagesSection({ content, sortOrder, eventId, guestId }: MessagesSectionProps) {
  ...
  return (
    <SectionWrapper sectionType="messages" sortOrder={sortOrder} className="flex flex-col justify-start py-8">
      {/* Title */}
      ...
      <div className="w-full max-w-[320px] flex flex-col gap-4 text-left my-auto">
        {/* Form Container */}
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-2.5 bg-white/40 p-4 rounded-xl border border-[var(--color-accent)]/15">
          <div>
            <input
              type="text"
          ...
          {submitStatus === 'success' && (
            <p className="text-[10px] text-green-600 font-semibold">Ucapan berhasil terkirim!</p>
          )}
          {submitStatus === 'error' && (
          ...
          <button
            ...
            className="w-full rounded-full bg-[var(--color-primary)] py-1.5 text-xs font-semibold text-white shadow-xs hover:opacity-90 active:scale-95 disabled:opacity-50 cursor-pointer"
          >
        ...
        {/* List Container */}
        <div className="bg-white/40 p-4 rounded-xl border border-[var(--color-accent)]/15 flex flex-col gap-2">
          <h3 className="text-[11px] font-semibold text-[var(--color-primary)] uppercase tracking-wider">
            Ucapan Doa Tamu
          </h3>
          
          <div className="max-h-[120px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
            {isLoading && messages.length === 0 ? (
              <p className="text-[10px] text-muted-foreground text-center py-2">Memuat ucapan...</p>
            ) : messages.length === 0 ? (
              <p className="text-[10px] text-muted-foreground text-center py-2">Belum ada ucapan.</p>
            ) : (
              messages.map((msg) => (
                <div key={msg.id} className="bg-white/70 p-2 rounded-lg border border-gray-100/50">
                  <div className="flex justify-between items-center gap-2">
                    <span className="text-[10px] font-bold text-[var(--color-primary)] line-clamp-1">{msg.sender_name}</span>
                    <span className="text-[8px] text-muted-foreground">{new Date(msg.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</span>
                  </div>
                  <p className="text-[10px] text-[var(--color-text)]/90 mt-1 leading-relaxed">{msg.message_text}</p>
                </div>
              ))
            )}
          </div>
          ...
          {/* Mini Pagination */}
          {totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-[var(--color-accent)]/10 pt-2 mt-1">
              <button
                onClick={() => loadMessages(page - 1)}
                disabled={page <= 1 || isLoading}
                className="text-[9px] font-semibold text-[var(--color-primary)] disabled:opacity-30 cursor-pointer"
              >
                ← Prev
              </button>

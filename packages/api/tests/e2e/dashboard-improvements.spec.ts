...
test.describe('Dashboard Improvements E2E', () => {
  ...
  test('Guest List: Should have perPage selector and correct capacity indicator', async ({
    page,
    tenantA,
  }) => {
    ...
    await expect(page.locator('main h1').filter({ hasText: 'Daftar Tamu' })).toBeVisible({
      timeout: 15000,
    });
  ...
  test('Send Invitation: Should have pagination and perPage selector', async ({
    page,
    tenantA,
  }) => {
    // Create 15 guests
    ...
    await expect(page.locator('main h1').filter({ hasText: 'Kirim Undangan' })).toBeVisible({
      timeout: 15000,
    });
  ...
  test('RSVP Tracking: Should display table, pagination, search, group, and status filters', async ({
    ...
    await expect(page.locator('main h1').filter({ hasText: 'Tracking RSVP' })).toBeVisible({
      timeout: 15000,
    });

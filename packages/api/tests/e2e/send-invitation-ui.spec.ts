...
test.describe('Send Invitation UI E2E', () => {
  test('should support template modification and single guest sending workflow', async ({
    page,
    tenantA,
  }) => {
    // Add logging
    ...
    await expect(
      page.locator('tr:has-text("Guest E2E Send Test")').locator('span:has-text("Belum Dikirim")')
    ).toBeVisible();
    ...
    await page.fill(
      '#template-editor',
      'Kepada Yth. {nama_tamu}, hadirlah di pernikahan kami: {link_undangan}'
    );
    ...
    await page
      .locator('tr:has-text("Guest E2E Send Test")')
      .locator('button:has-text("Kirim WA")')
      .click();
    ...
    await expect(
      page.locator('tr:has-text("Guest E2E Send Test")').locator('span:has-text("Terkirim")')
    ).toBeVisible();

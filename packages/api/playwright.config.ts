...
// Define web servers conditionally
const webServers = [
  {
...
// Only start invitation client server if NOT running on CI (since it requires local sibling workspace)
if (!process.env.CI) {
  webServers.push({
...
export default defineConfig({
  testDir: './tests/e2e',
  testIgnore: process.env.CI ? ['**/screenshot-generator.spec.ts'] : [],
  timeout: 30000,
  expect: {
    timeout: 5000,
  },

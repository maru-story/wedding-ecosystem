...
export async function scannerRoutes(app: FastifyInstance, opts: ScannerRouteOptions) {
  ...
  // POST /scanner/logs - Endpoint to capture and log client-side errors (PUBLIC)
  app.post('/logs', async (request: FastifyRequest, reply) => {
    ...
    if (parsedBody.success) {
      request.log.error({ clientError: parsedBody.data }, 'Client-side error captured from scanner PWA');
    } else {
  ...
  app.register(async (protectedApp) => {
    ...
    protectedApp.put('/devices/:deviceId/heartbeat', async (request, reply) => {
      const user = request.user!;
    ...
    protectedApp.delete('/devices/:deviceId', async (request, reply) => {
      const user = request.user!;
      const paramsSchema = z.object({
        deviceId: z.string().uuid({ message: 'ID device tidak valid' }),
      });
    ...
    protectedApp.get('/devices/:eventId', async (request, reply) => {
      const user = request.user!;
      const paramsSchema = z.object({
    ...
    protectedApp.get('/guests/:eventId', async (request, reply) => {
      const user = request.user!;
      const paramsSchema = z.object({
        eventId: z.string().uuid({ message: 'ID event tidak valid' }),
      });

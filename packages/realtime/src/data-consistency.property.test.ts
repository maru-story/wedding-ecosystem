...
describe('Property 16: Real-time Data Consistency', () => {
  it('total_checked_in in broadcast always equals the DB countCheckInsByEvent value', async () => {
    await fc.assert(
      fc.asyncProperty(arbEventId, arbDbCounts, async (eventId, counts) => {
        ...
        const broadcastPayload = broadcaster.broadcastStats.mock.calls[0][1] as StatsUpdatedPayload;
  ...
  it('broadcast payload and returned value from calculateAndBroadcastStats are identical (no stale data)', async () => {
    await fc.assert(
      fc.asyncProperty(arbEventId, arbDbCounts, async (eventId, counts) => {
        ...
        const returnedPayload = await service.calculateAndBroadcastStats(eventId);
  ...
  it('for any sequence of check-in operations, each broadcast reflects the current DB state', async () => {
    await fc.assert(
      fc.asyncProperty(arbEventId, arbCheckInSequence, async (eventId, checkInCounts) => {
        const broadcaster = createMockBroadcaster();
        ...
        expect(broadcaster.broadcastStats).toHaveBeenCalledTimes(checkInCounts.length);
      }),
      { numRuns: 50 }
    );
  });
